...
class Block:
    ...
    def _update_children_positions(self, position: Tuple[int, int]) -> None:
        ...
        if self.children != []:
            for i, child in enumerate(self.children):
                child._update_children_positions(child_pos[i])
    ...
    def smash(self) -> bool:
        ...
        """
        #Must be smashed
        #Then create 4 sub-children
        #Then use randomness for each children to smash again or not
        #Use recursion
        #If child is not smashed then assign it random colours
        child_pos = self._children_positions()
        ...
        if self.smashable():
            ...
            while i < 4:
                ...
                col = COLOUR_LIST[j]
                b = Block(child_pos[i], child_size, col, cur_lev+1, max_)
                children.append(b)
                i += 1

            for child in children:
                #For each children, check if they are smashed
                num = random.random()
                if num < math.exp(-0.25*cur_lev):
                    #If smashed, use recursion
                    child.smash()
    ...
    def swap(self, direction: int) -> bool:
        ...
        if self.children == []:
            ...
            if direction == 1:
                self.children[0].position = pos[3]
                self.children[1].position = pos[2]
                self.children[2].position = pos[1]
                self.children[3].position = pos[0]
                ...
                self.children[0].position = pos[1]
                self.children[1].position = pos[0]
                self.children[2].position = pos[3]
                self.children[3].position = pos[2]
        ...
        if self.children == []:
            ...
            if direction == 3:
                ...
                while i < len(self.children):
                    self.children[j].position = pos[i]
                    new.append(self.children[j])
                    i += 1
                    j += 1
                self.children = new
                for child in self.children:
                ...
                for child in self.children:
                    child.rotate(1)
                return True
            #Look to make these functions recursive and add a checker for debug?
    ...
    def combine(self) -> bool:
        ...
        if self.level != self.max_depth-1 or len(self.children) == 0:
            ...
            #gets children colors
            child_col = []
            for child in self.children:
                child_col.append(child.colour)
            count = []
            col_count = []
            i = 0
            #Organizes number of colours and the colours into seperate lists
            while i < len(child_col):  
                count.append(child_col.count(child_col[0]))
                col_count.append(child_col[0])
                child_col = [col for col in child_col if col != child_col[0]]
            #If 4 children colours or a tie for 2 and 2 then dont do anything
            if len(count) == 4:
                return False
            elif len(count) == 2 and count[0] == 2:
                return False
            #if no tie set the block color to the majority child colour
            else:
                max_ = max(count)
                index = count.index(max_)
                self.colour = col_count[index]
                self.children = []
                return True
    ...
    def create_copy(self) -> Block:
        ...
        b = Block(position, size, colour, level, max_depth)
        b.children = []
        for child in self.children:
            b.children.append(child.create_copy())
        return b
